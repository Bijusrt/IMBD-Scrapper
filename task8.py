...
test=os.path.exists('movies_dict/'+'TASK1'+'.json')
if test:
	with open('movies_dict/'+'TASK1'+'.json','r') as file:
		_list=json.loads(file.read())
	...
	for i in body:
		...
		_dict['Id']=i.find('div',attrs={'data-titleid':True})['data-titleid']
		_dict['Link']="https://www.imdb.com"+i.find('a')['href']
		_list.append(_dict)



	with open('movies_dict/'+'TASK1'+'.json','w+') as file:
		file.write(json.dumps(_list,indent=4))
...
if run[4] in string.digits:
	...
	return(movie_dict)
for i in _list:
	test=os.path.exists('movies_dict/'+i['Id']+'.json')
	if test:
		with open('movies_dict/'+i['Id']+'.json','r') as file:
	
			var3=json.loads(file.read())
			pprint.pprint(var3)
	else:
		with open('movies_dict/'+i['Id']+'.json','w+') as file:
			j=scrapped_movie(i['Link'])
